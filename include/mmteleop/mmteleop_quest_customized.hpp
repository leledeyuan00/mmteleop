...
private:
    ...
    // clutch: pose / resume one arm without leaving the teleop task.
    // Trigger toggles, so the state lives here and nowhere else.
    rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr pause_srv_l_;
    rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr pause_srv_r_;
    bool paused_l_{false};
    bool paused_r_{false};
    bool prev_paused_l_{false};
    bool prev_paused_r_{false};
    // Pose the increments are measured fram. Replaces the tasks's atart_pose so
    // that a resume can re-base the motion where the actually stopped.
    geometry_msgs::msg::PoseStamped base_pose_l_;
    geometry_msgs::msg::PoseStamped base_pose_r_;
    geometry_msgs::msg::PoseStamped last_target_l_;
    geometry_msgs::msg::PoseStamped last_target_r_;
