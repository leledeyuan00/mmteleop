    ...
    start_service_ = this->create_service<std_srvs::srv::SetBool>(
        ...
        [this](const std::shared_ptr<std_srvs::srv::SetBool::Request> request, std::shared_ptr<std_srvs::srv::SetBool::Response> response) -> void
        {
            if (request->data)
            {
                ...
                right_start_pose_ = right_current_pose_;
                left_start_pose_ = left_current_pose_;
                recorded_id_ = ids_[0]; // Temporary
            }
            else
            {
...
void MMTeleop::body_arrary_callback(const visualization_msgs::msg::MarkerArray::SharedPtr msg)
{
    ...
    {
        ids_[i] = msg->markers[i].id;
        if (msg->markers[i].id == recorded_id_)
        {
            tracking_body_index = i;
        }
    }
    ...
    for (size_t i = tracking_body_index; i < MARKER_NUM; i++)
    {
        Eigen::Vector3d marker_position;
        marker_position << msg->markers[i].pose.position.x, msg->markers[i].pose.position.y, msg->markers[i].pose.position.z;
...
void MMTeleop::get_hand_position(std::vector<Eigen::Vector3d> body_positions, Eigen::Vector3d& right_hand_position, Eigen::Vector3d& left_hand_position)
{
    ...
    // Eigen::Vector3d neck_z_axis = (left2upper_vector - neck_y_axis * neck_y_axis.dot(left2upper_vector)/ neck_y_axis.squaredNorm()).normalized(); // Neck z axis
    ...
    tf_broadcaster_->sendTransform(neck_transform);

    // std::cout << "neck rotation matrix : \r\n" << neck_rotation_matrix << std::endl;
    ...
    for (size_t i = 0; i < 3; i++)
    {
        ...
        left_hand_position(i) = filters::exponentialSmoothing(left_hand_position(i), left_hand_position_(i), 0.1); 
    }
    
    // right_hand_position =  (body_positions[0] - neck_middle_position);
    // left_hand_position =  (body_positions[1] - neck_middle_position);
}
...
void MMTeleop::loop()
{
    ...
    while (rclcpp::ok())
    {
        ...
        if (tracking_ready_ && initialized_r_ && initialized_l_)
        {
            ...
            Eigen::Vector3d right_hand_increment = right_hand_position_ - right_hand_position_start_;
            Eigen::Vector3d left_hand_increment = left_hand_position_ - left_hand_position_start_;

            // std::cout << "right hand increment : \r\n" << right_hand_increment << std::endl;
            // std::cout << "left hand increment : \r\n" << left_hand_increment << std::endl;
            ...
            right_pose_pub_->publish(right_target_pose);
            left_pose_pub_->publish(left_target_pose);
        }
